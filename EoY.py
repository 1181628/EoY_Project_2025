...
themes = {
    ...
    "inform":  {"font": ("Impact", 14), "bg": "#f2d243", "fg": "#2f58e0"},
    "button":  {"font": ("Impact", 22), "bg": "#2f58e0", "fg": "white", "width": 29},
    "catalogue": {"font": ("Impact", 14), "bg": "#2f58e0", "fg": "white", "width": 25},
...
image_dictionary = {
    "Blazegolem": Blazegolem,
    "Dawnmirage": Dawnmirage,
    "Frostste": Frostste,
    "Moldvine": Moldvine,
    "Vexscream": Vexscream,
    "Vortexwing": Vortexwing,
    "Wispghoul": Wispghoul}
...
def label_catalogue(action):
    ...
    frames["catalogue"].place(x=40, y=119, height=511, width=260)
    ...
    for name, stats in general_catalogue.items():
        ...
        cards_button.place(x=12, y=10+card_num*50)
        if action == 5:
    ...
    for each in range(maximum_cards):
        if each >= card_num:
            laybel = tk.Label(frames["catalogue"], text="Empty Card Box",
                font=themes["catalogue"]["font"], bg=themes["catalogue"]["bg"], fg=themes["catalogue"]["fg"], width=24, borderwidth=8)
            laybel.place(x=12, y=10+card_num*50)
            card_num = card_num + 1
...
subtitle_label = tk.Label(frames["title"], text="Choose what you want to do:",
    font=themes["title"]["font"], bg=themes["title"]["bg"], fg=themes["title"]["fg"])
subtitle_label.place(x=67, y=60)
...
exit_button = tk.Button(frames["button"], text="Save & Exit", command=save_exit,
    font=themes["button"]["font"], bg=themes["button"]["bg"], fg=themes["button"]["fg"], width=17, relief=themes["relief"])
